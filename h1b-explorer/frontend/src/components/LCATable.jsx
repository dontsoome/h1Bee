  ...
  if (!loading && records.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-400">No LCA records found.</p>
      </div>
    )
  }
        ...
        <thead>
          <tr className="bg-gray-50 border-b border-gray-100">
            {RECORD_COLUMNS.map((col) => (
              <th
                key={col.key}
                className={`px-4 py-3 font-medium text-gray-500 uppercase text-xs whitespace-nowrap ${col.className}`}
              >
                {col.label}
              </th>
            ))}
                ...
                <tr key={i} className="border-b border-gray-100">
                  {RECORD_COLUMNS.map((col) => (
                    <td key={col.key} className="px-4 py-3">
                      <div className="h-4 bg-gray-100 animate-pulse rounded" style={{ width: col.key === 'job_title' ? '160px' : '80px' }} />
                    </td>
                  ))}
                </tr>
              ))
...
function LCARecordRow({ record }) {
  ...
  return (
    <tr className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
      ...
      <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{location || '—'}</td>
      <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{record.visa_class || '—'}</td>
      <td className="px-4 py-3 text-right text-gray-600">{formatCurrency(record.annual_wage_from)}</td>
      <td className="px-4 py-3 text-right text-gray-600">{formatCurrency(record.annual_wage_to)}</td>
