...
function JobCardSkeleton() {
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 animate-pulse">
      ...
      <div className="mt-3 h-3 bg-gray-200 rounded w-full" />
...
function EmptyState({ onClear, hasFilters }) {
  return (
    <div className="text-center py-16">
      ...
      <h3 className="text-lg font-semibold text-gray-700 mt-4">No jobs found</h3>
      <p className="text-sm text-gray-400 mt-1">Try adjusting your filters or search terms</p>
      {hasFilters && (
        <button
          onClick={onClear}
          className="mt-3 text-purple-600 text-sm underline cursor-pointer hover:text-purple-700 transition-colors"
        >
          Clear all filters
        </button>
      )}
      ...
      */}
      ...
      <div className="flex-1 max-w-screen-2xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex gap-6 items-start">
            ...
            <div className="mb-4 flex items-center justify-between">
              {loading ? (
                ...
                <p className="text-sm text-gray-500">
                  Showing{' '}
                  <span className="font-medium text-gray-700">{jobs.length.toLocaleString()}</span>
                  {' '}of{' '}
              ...
              <button
                ...
                className="lg:hidden flex items-center gap-1.5 border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-50 transition-all"
              >
                ...
                Filters
                {activeFilterCount > 0 && (
                  <span className="bg-purple-600 text-white text-xs rounded-full px-1.5 ml-1 leading-5 min-w-[1.25rem] text-center">
                    {activeFilterCount}
                  </span>
                )}
              </button>
          ...
          <div className="hidden lg:flex w-[340px] xl:w-[380px] flex-shrink-0 sticky top-6 h-[calc(100vh-6rem)] rounded-xl overflow-hidden border border-gray-200 shadow-sm">
            <FilterPanel
              filters={filters}
