...
def get_companies(
    ...
    case_status: Optional[str] = "Certified",
    employer_name: Optional[str] = None,
    page: int = Query(default=1, ge=1),
    limit: int = Query(default=50, ge=1, le=100),
):
    """
    Return aggregated company stats from lca_records.
    Groups by employer_name and returns total_lcas, states, min/max/avg wages.
    """
    clauses: list[str] = []
    params: list = []

    # case_status filter (comma-separated) — defaults to "Certified" if not provided
    statuses = _split_csv(case_status) if case_status else ["Certified"]
    if statuses:
        placeholders = ",".join(["%s"] * len(statuses))
        clauses.append(f"case_status IN ({placeholders})")
        params.extend(statuses)

    # fiscal_year filter (comma-separated)
    years = _split_csv(fiscal_year)
    if years:
        # fiscal_year is stored as integer
        try:
    ...
    # worksite_state filter
    if state:
        clauses.append("worksite_state = %s")
        params.append(state)

    # pw_wage_level filter (comma-separated)
    ...
    data_sql = f"""
        SELECT
            employer_name,
            COUNT(*) AS total_lcas,
            STRING_AGG(DISTINCT worksite_state, ', ' ORDER BY worksite_state) AS states,
            MIN(annual_wage_from) AS min_wage,
            MAX(annual_wage_to) AS max_wage,
            ROUND(AVG(annual_wage_from)::numeric, 0) AS avg_wage
        FROM lca_records
        {where}
    ...
    # Convert Decimal/float to plain Python types for JSON serialization
    for row in rows:
        for key in ("min_wage", "max_wage", "avg_wage"):
            if row.get(key) is not None:
                row[key] = float(row[key])
        if row.get("total_lcas") is not None:
            row["total_lcas"] = int(row["total_lcas"])

    print(f"LCA companies query: {len(rows)} results (total_count={total_count}, page={page}, fiscal_year={fiscal_year!r}, case_status={case_status!r})")
    ...
    return {
        ...
        "page": page,
        "limit": limit,
        "total_pages": max(1, -(-int(total_count) // limit)),  # ceiling division
    }
