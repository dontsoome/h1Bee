...
def get_jobs(
    ...
    if is_remote is not None:
        clauses.append("is_remote = %s")
        params.append(is_remote)

    # Employment type filter
    emp_types = _split_csv(employment_type)
    if emp_types:
        placeholders = ",".join(["%s"] * len(emp_types))
        clauses.append(f"employment_type IN ({placeholders})")
        params += emp_types
    ...
    data_sql = f"""
        SELECT
            ...
            location, ats_platform, scraped_at::text, posted_at::text,
